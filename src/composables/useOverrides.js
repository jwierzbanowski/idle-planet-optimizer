...
export function useOverrides() {
  ...
  function getMiningLevel(id) { return overrides[id]?.miningLevel ?? 1 }
  function getMiningColonies(id) { return overrides[id]?.colonies ?? 0 }
  function getProbe(id) { return overrides[id]?.probe ?? false }
  function getProbeSpeed(id) { return overrides[id]?.probeSpeed ?? 0 }

  function setOverride(id, field, value) {
    if (!overrides[id]) overrides[id] = { stars: 0, market: 1, miningLevel: 1, colonies: 0, probe: false, probeSpeed: 0 }
