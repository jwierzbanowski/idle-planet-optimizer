...
export async function loadData() {
  ...
  for (const o of ores) { DB.value.ores[o.id] = { ...o, type: 'ore' }; order.ores.push(o.id) }
  for (const a of alloys) { DB.value.alloys[a.id] = { ...a, type: 'alloy' }; order.alloys.push(a.id) }
  for (const i of items) { DB.value.items[i.id] = { ...i, type: 'item' }; order.items.push(i.id) }
  for (const m of mining) { const id = 'lvl' + m.level; DB.value.mining[id] = { ...m, id, name: 'Level ' + m.level, type: 'mining' }; order.mining.push(id) }
  for (const p of planets) { DB.value.planets[p.id] = { ...p, type: 'planet' }; order.planets.push(p.id) }
