<template>
  <div class="star-controls" @click.stop>
    <button class="star-btn" :disabled="modelValue <= 0" @click="$emit('update:modelValue', modelValue - 1)">−</button>
    <span class="star-count">{{ modelValue }}</span>
    <button class="star-btn" @click="$emit('update:modelValue', modelValue + 1)">+</button>
  </div>
</template>
